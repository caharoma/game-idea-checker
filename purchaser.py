...
labels_ids = {'1k':0,'5k':1,'10k':2,'30k':3,'50k':4,'100k':5,'300k':6,'500k':7,'1m':8,'1.5m':9,'3m':10,'5m':11}
max_length = 60
batch_size=32

model_config = GPT2Config.from_pretrained(pretrained_model_name_or_path='model/gpt2_games_description_purchases/config.json')
tokenizer = GPT2Tokenizer.from_pretrained(pretrained_model_name_or_path='model/gpt2_games_description_purchases/tokenizer')
tokenizer.padding_size = "left"
tokenizer.pad_token = tokenizer.eos_token

model = GPT2ForSequenceClassification.from_pretrained(pretrained_model_name_or_path='model/gpt2_games_description_purchases/')
model.resize_token_embeddings(len(tokenizer))
model.config.pad_token_id = model.config.eos_token_id
model.to(device)

class DescriptionsDataset_predict(Dataset):
  ...
  def __getitem__(self, item):
    return {'text':self.text, 'label':'1k'}
...
#valid_dataset = DescriptionsDataset_predict(x,use_tokenizer=tokenizer)
...
def get_purchases(prompt):
  gpt2_classification_collator = Gpt2ClassificationCollator(use_tokenizer=tokenizer, max_sequence_len=max_length, labels_encoder=labels_ids)
  valid_dataset = DescriptionsDataset_predict(prompt,use_tokenizer=tokenizer)
  valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=False, collate_fn=gpt2_classification_collator)
  x = predict(valid_dataloader, device)
  y = list(labels_ids.keys())
  x = y[x]
  return(x)
